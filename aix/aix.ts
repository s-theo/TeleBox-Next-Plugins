...
type AIContentPart = { type: 'text'; text: string } | { type: 'image_url'; image_url: { url: string } }

type UnknownRecord = Record<string, unknown>
type UnknownPath = readonly (string | number)[]

const isUnknownRecord = (value: unknown): value is UnknownRecord => typeof value === 'object' && value !== null && !Array.isArray(value)

const getValueAtPath = (value: unknown, path: UnknownPath): unknown => {
  let current = value
...
const getStringAtPath = (value: unknown, path: UnknownPath): string | undefined => {
  const result = getValueAtPath(value, path)
  return typeof result === 'string' ? result : undefined
}

const getArrayAtPath = (value: unknown, path: UnknownPath): unknown[] => {
  const result = getValueAtPath(value, path)
...
const getErrorDetails = (error: unknown): ErrorDetails => {
  ...
  const response = isUnknownRecord(error.response)
    ? {
    ...
    : undefined
  const config = isUnknownRecord(error.config) ? error.config : undefined
  const signal = isUnknownRecord(config?.signal) ? config.signal : undefined
  return {
    ...
    config: config ? { signal: signal ? { reason: signal.reason } : undefined } : undefined,
    response
  }
}

const extractPromptOptimizeFlag = (
  prompt: string
): {
  prompt: string
...
type PromptOptimizationCommand = { type: 'status' } | { type: 'toggle'; enabled: boolean } | { type: 'length-status' } | { type: 'length'; mode: PromptLengthMode }
...
const parsePromptOptimizationCommand = (args: string[], feature: 'image' | 'video'): PromptOptimizationCommand => {
  ...
  if (option === 'length') {
    ...
    if (args.length === 4 && (mode === 'short' || mode === 'medium' || mode === 'long')) {
      return { type: 'length', mode }
    }
  }

  if (args.length === 3 && (option === 'short' || option === 'medium' || option === 'long')) {
    return { type: 'length', mode: option }
  }

  throw new UserError(buildCommandUsage(`aix ${feature} optimize [on|off|length [short|medium|long]]`))
}

const getPromptLengthInstruction = (mode: string | undefined, labels: typeof IMAGE_PROMPT_LENGTH_LABELS | typeof VIDEO_PROMPT_LENGTH_LABELS): string => {
  const normalized = normalizePromptLengthMode(mode)
  return `只输出提示词正文，不要解释、标题、Markdown、引号；中文输入就中文输出；控制在 ${labels[normalized]}。`
}

const buildMultiImagePromptPlanRequest = (prompt: string, count: number, lengthMode?: PromptLengthMode): string =>
  [
...
const parseMultiImagePromptPlan = (text: string, count: number): string[] => {
  ...
  const cleaned = raw
    ...
    .replace(/```$/i, '')
    .trim()
  const tryArray = (value: unknown): string[] => {
...
const buildImageRequirementAnalysisRequest = (prompt: string, lengthMode?: PromptLengthMode): string =>
  [
...
const parseImageRequirementAnalysis = (text: string): string[] => {
  ...
  const parseValue = (value: unknown): string[] => {
    const source = Array.isArray(value) ? value : isUnknownRecord(value) ? value.prompts : undefined
    if (!Array.isArray(source)) return []
    const prompts = source.map((item) => (typeof item === 'string' ? item.replace(/\s+/g, ' ').trim() : '')).filter(Boolean)
...
const buildStickerTextFallbackPrompts = (prompt: string, count: number): string[] => {
  ...
  return Array.from({ length: count }, (_, i) => `${prompt}，做成清晰可爱的社交平台表情包构图，主体表情夸张有辨识度，干净背景，高对比描边，在画面中加入醒目的中文短文字「${stickerTexts[i % stickerTexts.length]}」，文字要清楚可读并贴合表情。`)
}
...
const buildPromptOptimizationStatusText = (prompt: string): string => `🧠 <b>正在优化生图提示词...</b>\n\n` + `<b>📝 原始 Prompt</b>\n` + `<blockquote expandable>${htmlEscape(prompt)}</blockquote>`

const buildOptimizedPromptStatusText = (prompt: string): string => `✅ <b>提示词已优化，正在生图...</b>\n\n` + `<b>🖼️ 优化后 Prompt</b>\n` + `<blockquote expandable>${htmlEscape(prompt)}</blockquote>\n` + `<i>⏳ 这条优化结果会在 10 秒后自动删除</i>`

const buildMultiPromptStatusText = (title: string, prompts: string[]): string => `✅ <b>${htmlEscape(title)}</b>\n\n` + prompts.map((prompt, index) => `<b>${index + 1}.</b>\n<blockquote expandable>${htmlEscape(prompt)}</blockquote>`).join('\n\n') + `\n<i>⏳ 这条优化结果会在 10 秒后自动删除</i>`

const buildGeneratedPromptListText = (prompts: string[]): string => prompts.map((prompt, index) => `${index + 1}. ${htmlEscape(prompt)}`).join('\n\n')

const runAIWithTimeout = async (aiService: AIService, request: string, imageParts: AIContentPart[], parentToken: AbortToken, timeoutMs: number): Promise<{ text: string; images: AIImage[] }> => {
  const requestToken = aiService.createAbortToken()
  const abortFromParent = () => requestToken.abort(parentToken.reason || '操作已取消')
...
interface FeatureHandler {
  ...
  execute(msg: MessageContext, args: string[]): Promise<void>
}
...
const PROVIDER_TYPES = ['openai-compatible', 'openai', 'gemini', 'doubao', 'moonshot', 'local-cliproxy'] as const

type ProviderType = (typeof PROVIDER_TYPES)[number]
const PROVIDER_TYPE_OPTIONS = PROVIDER_TYPES.join('/')

type ProviderStrategy = 'openai-rest' | 'gemini-rest' | 'doubao-rest' | 'gemini-image-rest' | 'gemini-video-rest'
...
const mapHostsToProviderType = (hostList: string[], providerType: ProviderType): Record<string, ProviderType> => {
...
const createProviderProfile = (id: ProviderType, options: Omit<ProviderProfile, 'id'>): ProviderProfile => ({
  id,
  ...options
})

const createOpenAIProfile = (id: 'openai-compatible' | 'openai'): ProviderProfile =>
...
const isProviderType = (value: string): value is ProviderType => (PROVIDER_TYPES as readonly string[]).includes(value)
...
const resolveProviderType = (providerConfig?: Pick<ProviderConfig, 'url' | 'type'> | null): ProviderType => {
...
const getProviderProfile = (providerConfig?: Pick<ProviderConfig, 'url' | 'type'> | null): ProviderProfile => PROVIDER_PROFILES[resolveProviderType(providerConfig)]

const isOpenAIProviderType = (providerType: ProviderType): boolean => providerType === 'openai' || providerType === 'openai-compatible'

const formatProviderTypeLabel = (providerConfig: Pick<ProviderConfig, 'url' | 'type'>): string => {
  const configuredType = normalizeProviderType(providerConfig.type)
  if (configuredType) return configuredType
  return `auto -> ${resolveProviderType(providerConfig)}`
}

const mergeDefaults = <T extends { extraParams?: UnknownRecord }>(a?: T, b?: T): T | undefined => {
...
const resolveModeConfig = (profile: ProviderProfile, mode: ProviderMode, model: string): ProviderModeConfig | undefined => {
...
const resolveBaseUrl = (providerConfig: ProviderConfig, modeConfig: ProviderModeConfig): string => {
...
const resolveResponsesEndpointUrl = (providerConfig: ProviderConfig, modeConfig: ProviderModeConfig): string => {
  const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
  const currentUrl = modeConfig.endpoint ? resolveEndpointUrl(baseUrl, modeConfig.endpoint) : baseUrl
  const responsesBaseUrl = normalizeOpenAIBaseUrl(currentUrl)
  return resolveEndpointUrl(responsesBaseUrl, 'responses')
}

const getMessageText = (message?: Message | null): string => message?.text ?? ''

const htmlEscape = (text: string): string => text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')

const getMainPrefix = (): string => getPrefixes()[0] || ''

const buildCommandUsage = (command: string): string => `用法: <code>${htmlEscape(command.startsWith('aix') ? `${getMainPrefix()}${command}` : command)}</code>`

const buildMissingConfigMessage = (mode: ProviderMode): string => {
  const prefix = getMainPrefix()
  return `请先配置 API 并设置模型\n` + `使用 <code>${htmlEscape(`${prefix}aix config add <tag...> <url> <key> [type]`)}</code> ` + `和 <code>${htmlEscape(`${prefix}aix model ${mode} <tag...> <model-path>`)}</code>`
}
...
const extractErrorMessage = (error: unknown): string => {
  ...
  const msgText = typeof details.message === 'string' ? details.message : ''
  const reasonText = typeof details.cause === 'string' ? details.cause : details.cause ? String(details.cause) : details.config?.signal?.reason ? String(details.config.signal.reason) : ''
  ...
  if (details.name === 'AbortError' || msgText.toLowerCase().includes('aborted')) return '操作已取消'
  if (details.code === 'ECONNABORTED') return '请求超时'
  if (details.response?.status === 429) return '请求过于频繁，请稍后重试'
  return getStringAtPath(details.response?.data, ['error', 'message']) || getStringAtPath(details.response?.data, ['message']) || msgText || '未知错误'
}
...
function requireUser(condition: unknown, message: string): asserts condition {
...
const formatErrorForDisplay = (error: unknown): string => {
  const details = getErrorDetails(error)
  if (error instanceof UserError || details.name === 'AbortError' || (typeof details.message === 'string' && details.message.toLowerCase().includes('aborted'))) {
...
const sendErrorMessage = async (msg: MessageContext, error: unknown, trigger?: MessageContext): Promise<void> => {
...
type DocumentMedia = Extract<NonNullable<MessageMedia>, { type: 'document' | 'video' | 'sticker' | 'audio' | 'voice' }>

const getDocumentThumb = (doc: DocumentMedia): Thumbnail | undefined => {
  const thumbs = doc.thumbnails
  if (thumbs.length === 0) return undefined
  return thumbs[thumbs.length - 1]
}

const resolveImageInputs = async (parts: AIContentPart[], httpClient: HttpClient, token?: AbortToken, options?: { allowFailures?: boolean }): Promise<ResolvedImageData[]> => {
  const allowFailures = options?.allowFailures ?? false
  const imageParts = parts.filter((part): part is Extract<AIContentPart, { type: 'image_url' }> => part.type === 'image_url')

  const resolvePart = async (part: Extract<AIContentPart, { type: 'image_url' }>): Promise<ResolvedImageData | null> => {
    ...
    return image?.data ? { data: image.data, mimeType: image.mimeType } : null
  }
  ...
  return resolved.filter((item): item is ResolvedImageData => item !== null)
}

const resolveMergedImageParts = async (parts: AIContentPart[], httpClient: HttpClient, token?: AbortToken): Promise<AIImage | null> => {
  ...
  const renderedItems = await Promise.all(
    inputItems.map(async (item) => {
      const buf = await sharp(item.data).resize(cell, cell, { fit: 'inside', withoutEnlargement: true }).flatten({ background: 'white' }).png().toBuffer()
...
const isDocumentMedia = (media: MessageMedia): media is DocumentMedia => media !== null && ['document', 'video', 'sticker', 'audio', 'voice'].includes(media.type)

const collectImagePartsFromSingleMessage = async (msg: Message, client: TelegramClient, out: AIContentPart[]): Promise<void> => {
  const media = msg.media
  if (!media) return

  if (media.type === 'photo') {
    const buffer = Buffer.from(await client.downloadAsBuffer(media))
  ...
  if (isDocumentMedia(media)) {
    ...
    const isImageFile = docMime.startsWith('image/') || /\.(?:jpe?g|png|webp|gif|bmp|tiff?|avif|heic|heif)$/i.test(fileName)
    const isAnimated = docMime === 'image/gif' || docMime === 'video/webm' || docMime === 'application/x-tgsticker' || docMime === 'application/x-tg-sticker'

    const thumb = getDocumentThumb(doc)

    if (!isAnimated && isImageFile) {
      const buffer = Buffer.from(await client.downloadAsBuffer(doc))
    ...
    if (thumb) {
      const buffer = Buffer.from(await client.downloadAsBuffer(thumb))
      try {
...
const getMessageImageParts = async (msg?: Message): Promise<AIContentPart[]> => {
  if (!msg) return []

  const client = await getGlobalClient()
  const parts: AIContentPart[] = []
  const groupedId = msg.groupedId ? String(msg.groupedId) : undefined

  if (!groupedId) {
    await collectImagePartsFromSingleMessage(msg, client, parts)
    return parts
  }

  let sameGroupMessages: Message[]
  if (msg instanceof MessageContext) {
    sameGroupMessages = msg.messages.filter((message) => String(message.groupedId) === groupedId)
  } else {
    const nearbyIds = Array.from({ length: 19 }, (_, index) => msg.id - 9 + index).filter((id) => id > 0)
    const nearbyMessages = await client.getMessages(msg.chat.id, nearbyIds)
    sameGroupMessages = nearbyMessages.filter((message): message is Message => message !== null && String(message.groupedId) === groupedId)
  }
  ...
  const groupedParts = await Promise.all(
    sameGroupMessages.map(async (message) => {
      ...
      await collectImagePartsFromSingleMessage(message, client, messageParts)
      return messageParts
    })
  )
...
const downloadAvatarBufferCompat = async (msg?: Message): Promise<Buffer | null> => {
  ...
  try {
    const client = await getGlobalClient()
    const photos = await withTimeout(client.getProfilePhotos(msg.sender, { limit: 1 }), 12000)
    const photo = photos?.[0]
    if (!photo) return null
    const buffer = Buffer.from(await client.downloadAsBuffer(photo))
...
const getAvatarImagePartFromMessageSender = async (msg?: Message): Promise<AIContentPart | null> => {
...
const deleteStatusMessage = async (msg?: Message): Promise<void> => {
  if (!msg) return
  try {
    const client = await getGlobalClient()
    await client.deleteMessagesById(msg.chat.id, [msg.id], { revoke: true })
  } catch {}
}

const replaceStatusMessage = async (current: Message | undefined, msg: MessageContext, text: string): Promise<Message> => MessageSender.sendOrEdit(current ?? msg, text)
...
const resolveAIImageData = async (image: AIImage, httpClient: HttpClient, token?: AbortToken): Promise<AIImage | null> => {
  ...
  const contentType = getHeaderContentType(response.headers) || image.mimeType || 'image/jpeg'
...
const resolveAIVideoData = async (video: AIVideo, httpClient: HttpClient, token?: AbortToken): Promise<AIVideo | null> => {
  ...
  const contentType = getHeaderContentType(response.headers) || video.mimeType || 'video/mp4'
  return { data: Buffer.from(response.data), mimeType: contentType }
}

const videoHasAudioTrack = async (filePath: string): Promise<boolean> => {
  try {
    const { stdout } = await execFileAsync('ffprobe', ['-v', 'error', '-show_streams', '-select_streams', 'a:0', '-of', 'json', filePath])
...
const ensureVideoHasAudio = async (inputPath: string, outputPath: string): Promise<string> => {
  try {
    ...
    await execFileAsync('ffmpeg', ['-y', '-i', inputPath, '-f', 'lavfi', '-i', 'anullsrc=channel_layout=stereo:sample_rate=44100', '-c:v', 'copy', '-shortest', '-c:a', 'aac', '-b:a', '128k', outputPath])
...
const runOrderedJobs = async <T>(count: number, concurrency: number, token: AbortToken | undefined, job: (index: number) => Promise<T>): Promise<T[]> => {
  if (count <= 0) return []
...
const retryWithFixedDelay = async <T>(operation: () => Promise<T>, maxRetries: number = 2, delayMs: number = 1000, token?: AbortToken): Promise<T> => {
...
const isRetryableError = (error: unknown): boolean => {
  ...
  if (typeof details.message === 'string' && details.message.toLowerCase().includes('aborted')) return false
  ...
  if (details.isAxiosError && !details.response) return true
  if (typeof details.code === 'string') return true
...
type TaskFetchFn = (token?: AbortToken) => Promise<unknown>
type TaskParseFn<T> = (data: unknown) => TaskPollResult<T>

const pollTask = async <T>(fetchJob: TaskFetchFn, parseResult: TaskParseFn<T>, options: TaskPollOptions = {}, token?: AbortToken): Promise<T> => {
...
const getEditErrorText = (error: unknown): string => {
  const details = typeof error === 'object' && error !== null ? (error as { errorMessage?: unknown; message?: unknown }) : undefined
  const parts = [typeof details?.errorMessage === 'string' ? details.errorMessage : '', typeof details?.message === 'string' ? details.message : ''].filter(Boolean)
  return parts.join(' ')
}

const isMessageNotModifiedError = (error: unknown): boolean => getEditErrorText(error).includes('MESSAGE_NOT_MODIFIED')

const shouldFallbackToReplyOnEditError = (error: unknown): boolean => {
  const text = getEditErrorText(error)
  return text.includes('MESSAGE_ID_INVALID') || text.includes('MESSAGE_AUTHOR_REQUIRED')
}

const getTopicRootId = (msg: MessageContext): number | undefined => {
  return msg.replyToMessage?.threadId ?? undefined
}

const MessageSender = {
  async sendOrEdit(msg: Message, text: string, options?: MessageOptions): Promise<Message> {
    const client = await getGlobalClient()
    try {
      ...
      if (shouldFallbackToReplyOnEditError(error)) {
        const replied = await client.sendText(msg.chat.id, html(text), {
          replyTo: msg.id,
          ...options
        })
  ...
  async sendNew(msg: Message, text: string, options?: MessageOptions, replyToId?: number): Promise<Message> {
    const client = await getGlobalClient()
    const topicRootId = msg instanceof MessageContext ? getTopicRootId(msg) : undefined
    return await client.sendText(msg.chat.id, html(text), {
      ...(options || {}),
      ...(replyToId ? { replyTo: replyToId } : {}),
      ...(!replyToId && topicRootId ? { threadId: topicRootId } : {})
    })
  }
}
...
class MessageUtils {
  ...
  async createTelegraphPage(markdown: string, titleSource?: string, token?: AbortToken): Promise<TelegraphItem> {
    ...
    const url = getStringAtPath(response.data, ['result', 'url'])
    if (!url) throw new Error(getStringAtPath(response.data, ['error']) || 'Telegraph 页面创建失败')

    return { url, title, createdAt: new Date().toISOString() }
  }

  async sendLongMessage(msg: MessageContext, text: string, replyToId?: number, token?: AbortToken, options?: { poweredByTag?: string }): Promise<Message> {
    token?.throwIfAborted()
  ...
  async sendImages(msg: MessageContext, images: AIImage[], prompt: string | string[], replyToId?: number, token?: AbortToken, notice?: string): Promise<void> {
  ...
  async sendVideos(msg: MessageContext, videos: AIVideo[], prompt: string, replyToId?: number, token?: AbortToken): Promise<void> {
    const config = (await this.configManagerPromise).getConfig()
  ...
  private async sendMedia<T extends AIImage | AIVideo>(
    ...
    const combinedPrompt = Array.isArray(prompt) ? prompt.map((text, index) => `${index + 1}. ${text}`).join('\n\n') : prompt
    ...
    if (mediaItems.length > 1) {
      ...
      try {
        const preparedPaths = await runOrderedJobs(mediaItems.length, 2, token, async (i) => {
          ...
          const pathToSend = options.prepareForSend ? await options.prepareForSend(rawPath, finalPath) : rawPath
          if (options.prepareForSend) cleanupAll.push(finalPath)
          return pathToSend
        })
        const pathsToSend = preparedPaths.filter((item): item is string => item !== null)
        if (pathsToSend.length > 1) {
          const topicRootId = getTopicRootId(msg)
          const mediaType: 'photo' | 'video' | 'document' = !options.previewEnabled ? 'document' : options.prepareForSend ? 'video' : 'photo'
          const groupMedia: InputMediaLike[] = pathsToSend.map((filePath, index) => ({
            type: mediaType,
            file: `file:${filePath}`,
            fileName: path.basename(filePath),
            ...(index === 0 ? { caption: html(caption) } : {})
          }))
          await client.sendMediaGroup(peerId, groupMedia, {
            ...(replyToId ? { replyTo: replyToId } : {}),
            ...(!replyToId && topicRootId ? { threadId: topicRootId } : {})
          })
          if (options.saveToFavorites) {
            try {
              const favoritesMedia: InputMediaLike[] = pathsToSend.map((filePath) => ({
                type: 'document',
                file: `file:${filePath}`,
                fileName: path.basename(filePath)
              }))
              await client.sendMediaGroup('me', favoritesMedia)
            } catch (error) {
    ...
    for (let i = 0; i < mediaItems.length; i++) {
      ...
      try {
        ...
        await client.sendMedia(
          ...
          {
            type: mediaType,
            file: `file:${pathToSend}`,
            fileName: path.basename(pathToSend),
            caption: html(itemCaptions[i] ?? caption)
          },
          {
            ...(replyToId ? { replyTo: replyToId } : {}),
            ...(!replyToId && topicRootId ? { threadId: topicRootId } : {})
  ...
  private async ensureTGToken(config: DB, token?: AbortToken): Promise<string> {
    ...
    this.telegraphTokenPromise = (async () => {
      ...
      const tgToken = getStringAtPath(response.data, ['result', 'access_token'])
  ...
  private async sendHtml(msg: MessageContext, content: string, replyToId?: number, linkPreview?: boolean): Promise<Message> {
    return await MessageSender.sendNew(
      msg,
      content,
      {
        ...(linkPreview === undefined ? {} : { disableWebPreview: !linkPreview })
      },
      replyToId
    )
  }
  ...
  private ensureDefaults(): void {
    ...
    if (!cfg.telegraph || typeof cfg.telegraph !== 'object') {
      ...
      if (!Array.isArray(cfg.telegraph.list)) {
        ...
        cfg.telegraph.list = cfg.telegraph.list.filter((item): item is TelegraphItem => !!item && typeof item.url === 'string' && typeof item.title === 'string' && typeof item.createdAt === 'string')
      }
    }
  }
}

const resolveAuthMode = (profile: ProviderProfile, modeConfig: ProviderModeConfig, config?: ProviderConfig): AuthMode => {
...
const applyAuthConfig = (authMode: AuthMode, config: ProviderConfig, url: string, headers: Record<string, string>): { url: string; headers: Record<string, string> } => {
...
const normalizeOpenAIBaseUrl = (url: string): string => {
  try {
    ...
    const stripSuffixes = ['/chat/completions', '/completions', '/responses', '/messages', '/images/generations']
...
const parseOpenAIChatResponse = (data: unknown): { text: string; images: AIImage[] } => {
  const parseContent = (content: unknown): { text: string; images: AIImage[] } => {
    ...
    for (const part of parts) {
      ...
      const imageUrl = getStringAtPath(part, ['image_url', 'url'])
      if (part.type === 'image_url' && imageUrl) {
        ...
        else images.push({ url: imageUrl, mimeType: 'image/jpeg' })
      }
    }
  ...
  const message = getValueAtPath(data, ['choices', 0, 'message'])
  if (!message) return { text: 'AI 回复为空', images: [] }
  return parseContent(getValueAtPath(message, ['content']))
}

const parseOpenAIStyleImageResponse = (data: unknown): AIImage[] => {
  const images: AIImage[] = []
  const list = getArrayAtPath(data, ['data'])
  for (const item of list) {
    const base64 = getStringAtPath(item, ['b64_json'])
    const url = getStringAtPath(item, ['url'])
    if (base64) {
      images.push({
...
const isAsyncIterable = (value: unknown): value is AsyncIterable<unknown> => typeof value === 'object' && value !== null && Symbol.asyncIterator in value && typeof value[Symbol.asyncIterator] === 'function'

const readResponseBodyAsText = async (data: unknown): Promise<string> => {
...
const collectOpenAISources = (data: unknown): Array<{ url: string; title?: string }> => {
  const sources: Array<{ url: string; title?: string }> = []
  const seen = new Set<string>()

  const appendEntries = (entries: unknown, isAnnotation = false) => {
    if (!Array.isArray(entries)) return
    for (const entry of entries) {
      const url = isAnnotation ? getStringAtPath(entry, ['url_citation', 'url']) || getStringAtPath(entry, ['url']) : getStringAtPath(entry, ['url'])
      const title = isAnnotation ? getStringAtPath(entry, ['url_citation', 'title']) || getStringAtPath(entry, ['title']) : getStringAtPath(entry, ['title'])
  ...
  const choice = getValueAtPath(data, ['choices', 0])

  appendEntries(getValueAtPath(data, ['citations']))
  appendEntries(getValueAtPath(choice, ['citations']))
  appendEntries(getValueAtPath(choice, ['message', 'citations']))
  appendEntries(getValueAtPath(choice, ['delta', 'citations']))

  appendEntries(getValueAtPath(data, ['annotations']), true)
  appendEntries(getValueAtPath(choice, ['message', 'annotations']), true)
  appendEntries(getValueAtPath(choice, ['delta', 'annotations']), true)
...
const aggregateOpenAIResponses = (
  ...
  for (const payload of payloads) {
    const choice = getValueAtPath(payload, ['choices', 0])
    const deltaContent = getValueAtPath(choice, ['delta', 'content'])
    ...
    const fallbackContent = getValueAtPath(choice, ['message', 'content']) ?? getValueAtPath(choice, ['content']) ?? getValueAtPath(payload, ['content'])
    ...
    } else if (getStringAtPath(choice, ['text'])?.trim()) {
      fallbackText = getStringAtPath(choice, ['text'])?.trim() ?? ''
      fallbackImages = []
    } else if (getStringAtPath(payload, ['text'])?.trim()) {
      fallbackText = getStringAtPath(payload, ['text'])?.trim() ?? ''
      fallbackImages = []
    }
  ...
  const text = (deltaTexts.length > 0 ? deltaTexts.join('').trim() : fallbackText.trim()) || 'AI 回复为空'
  const images = deltaImages.length > 0 ? deltaImages : fallbackImages

  return { text, images, sources }
}

const collectResponsesSources = (item: unknown): Array<{ url: string; title?: string }> => {
  const sources: Array<{ url: string; title?: string }> = []
  const seen = new Set<string>()
  ...
  const appendAnnotations = (annotations: unknown) => {
    if (!Array.isArray(annotations)) return
    for (const entry of annotations) {
      if (getStringAtPath(entry, ['type']) !== 'url_citation') continue
      appendSource(getStringAtPath(entry, ['url']), getStringAtPath(entry, ['title']))
    }
  }

  const appendActionSources = (action: unknown) => {
    const actionSources = getValueAtPath(action, ['sources'])
    const sourceList = Array.isArray(actionSources) ? actionSources : Array.isArray(action) ? action : []
    for (const entry of sourceList) {
      const url = getStringAtPath(entry, ['url'])
      if (!url) continue
      appendSource(url, getStringAtPath(entry, ['title']))
    }
  }

  if (getStringAtPath(item, ['type']) === 'message') {
    for (const part of getArrayAtPath(item, ['content'])) {
      appendAnnotations(getValueAtPath(part, ['annotations']))
    }
  }

  if (getStringAtPath(item, ['type']) === 'web_search_call') {
    const action = getValueAtPath(item, ['action'])
    if (Array.isArray(action)) {
      for (const entry of action) appendActionSources(entry)
    } else {
      appendActionSources(action)
    }
  }
...
const parseResponsesOutputContent = (
  ...
  for (const part of getArrayAtPath(item, ['content'])) {
    const partType = getStringAtPath(part, ['type'])
    const partText = getStringAtPath(part, ['text'])
    if (partType === 'output_text' && partText) {
...
const aggregateResponsesApiPayloads = (
  ...
  for (const payload of payloads) {
    const payloadType = getStringAtPath(payload, ['type'])
    const payloadDelta = getStringAtPath(payload, ['delta'])
    if (payloadType === 'response.output_text.delta' && payloadDelta) {
      deltaTexts.push(payloadDelta)
    }

    const payloadText = getStringAtPath(payload, ['text'])
    if (payloadType === 'response.output_text.done' && payloadText && deltaTexts.length === 0) {
      fallbackText = payloadText.trim()
    }

    if (payloadType === 'response.content_part.done') {
      appendSources(
        collectResponsesSources({
          type: 'message',
          content: [getValueAtPath(payload, ['part'])]
        })
      )
    }

    const payloadItem = getValueAtPath(payload, ['item'])
    if (payloadItem) appendItem(payloadItem)

    const nestedResponse = getValueAtPath(payload, ['response'])
    const response = getStringAtPath(nestedResponse, ['object']) === 'response' ? nestedResponse : getStringAtPath(payload, ['object']) === 'response' ? payload : null
    const output = getValueAtPath(response, ['output'])
  ...
  return {
    text: (deltaTexts.length > 0 ? deltaTexts.join('').trim() : fallbackText.trim()) || 'AI 回复为空',
    images: fallbackImages,
    sources
  }
}

const parseOpenAIResponsePayloads = (raw: string): unknown[] => {
...
const parseOpenAIResponseData = async (
  ...
  if (data && typeof data === 'object' && !Buffer.isBuffer(data) && !(data instanceof Uint8Array) && !isAsyncIterable(data)) {
    if (getStringAtPath(data, ['object']) === 'response') {
  ...
  if (payloads.length > 0) {
    const hasResponsesPayload = payloads.some((payload) => getStringAtPath(payload, ['object']) === 'response' || getStringAtPath(payload, ['response', 'object']) === 'response' || getStringAtPath(payload, ['type'])?.startsWith('response.') === true)
    return hasResponsesPayload ? aggregateResponsesApiPayloads(payloads) : aggregateOpenAIResponses(payloads)
  }

  return { text: raw.trim() || 'AI 回复为空', images: [], sources: [] }
}

const buildDoubaoVideoUrl = (data: unknown): string | null => {
  return (
    getStringAtPath(data, ['data', 'result', 'video_url']) ||
    getStringAtPath(data, ['data', 'output', 'video_url']) ||
    getStringAtPath(data, ['data', 'video_url']) ||
    getStringAtPath(data, ['video_url']) ||
...
const extractGeminiOperationError = (data: unknown): string => {
  const err = getValueAtPath(data, ['error']) || getValueAtPath(data, ['data', 'error'])
  if (!err) return ''
  if (typeof err === 'string') return err
  const message = getStringAtPath(err, ['message'])
  if (message) return message
  const status = getStringAtPath(err, ['status'])
  if (status) return status
  const details = getArrayAtPath(err, ['details'])
  if (details.length > 0) {
    const detailMessage = getStringAtPath(details[0], ['message'])
    if (detailMessage) return detailMessage
  }
  return '视频生成失败'
}

const extractGeminiVideoResult = (data: unknown): { uri?: string; bytes?: string } | null => {
  const response = getValueAtPath(data, ['response']) ?? getValueAtPath(data, ['data', 'response']) ?? data
  const sampleUri = getStringAtPath(response, ['generateVideoResponse', 'generatedSamples', 0, 'video', 'uri']) || getStringAtPath(response, ['generate_video_response', 'generated_samples', 0, 'video', 'uri'])
  if (sampleUri) return { uri: sampleUri }

  const videoBytes =
    getStringAtPath(response, ['generatedVideos', 0, 'video', 'videoBytes']) ||
    getStringAtPath(response, ['generated_videos', 0, 'video', 'video_bytes']) ||
    getStringAtPath(response, ['generatedVideos', 0, 'video', 'video_bytes']) ||
    getStringAtPath(response, ['generated_videos', 0, 'video', 'videoBytes'])
  if (videoBytes) return { bytes: videoBytes }

  return null
}

const buildGeminiParts = async (prompt: string, images: AIContentPart[], httpClient: HttpClient, token?: AbortToken): Promise<UnknownRecord[]> => {
...
class HttpClient {
  ...
  constructor(private readonly configManagerPromise: Promise<ConfigManager>) {
  ...
  async request<T = unknown>(requestConfig: AxiosRequestConfig, token?: AbortToken): Promise<AxiosResponse<T>> {
    const timeoutMs = (await this.configManagerPromise).getConfig().timeout * 1000
    const requestToken = createAbortToken()
    const abortFromExternal = () => requestToken.abort(token?.reason || '操作已取消')
    ...
    const timeoutId = setTimeout(() => requestToken.abort(`请求超时: ${timeoutMs}ms`), timeoutMs)
    try {
...
class AIService {
  ...
  private async getCurrentProviderConfig(type: ProviderMode): Promise<{ providerConfig: ProviderConfig; model: string; config: DB }> {
    const configManager = await this.configManagerPromise
    const config = configManager.getConfig()
  ...
  private resolveMode(providerConfig: ProviderConfig, mode: ProviderMode, model: string): { profile: ProviderProfile; modeConfig: ProviderModeConfig } {
  ...
  private applyImageDefaults(request: UnknownRecord, providerConfig: ProviderConfig, model: string, modeConfig: ProviderModeConfig): void {
  ...
  private applyVideoDefaults(request: UnknownRecord, modeConfig: ProviderModeConfig): void {
  ...
  private createStrategyHandlers(): Record<ProviderStrategy, StrategyHandler> {
    return {
      'openai-rest': {
        chat: async (ctx) => this.callOpenAIChatOrSearch(ctx.providerConfig, ctx.model, ctx.question, ctx.images, ctx.modeConfig, ctx.config.prompt || '', ctx.token),
        search: async (ctx) => this.callOpenAIChatOrSearch(ctx.providerConfig, ctx.model, ctx.question, ctx.images, ctx.modeConfig, ctx.config.prompt || '', ctx.token, true),
        image: async (ctx) => this.generateImageWithOpenAIRest(ctx.providerConfig, ctx.model, ctx.prompt, ctx.image, ctx.modeConfig, ctx.token),
        video: async (ctx) => this.generateVideoWithOpenAIRest(ctx.providerConfig, ctx.model, ctx.prompt, ctx.images, ctx.modeConfig, ctx.token)
      },
      'gemini-rest': {
        chat: async (ctx) => this.callGeminiChatOrSearch(ctx.providerConfig, ctx.model, ctx.question, ctx.images, ctx.modeConfig, ctx.config.prompt || '', ctx.token),
        search: async (ctx) => this.callGeminiChatOrSearch(ctx.providerConfig, ctx.model, ctx.question, ctx.images, ctx.modeConfig, ctx.config.prompt || '', ctx.token, true),
        image: async (ctx) => this.generateGeminiImageRest(ctx.providerConfig, ctx.model, ctx.prompt, ctx.modeConfig, ctx.image, ctx.token)
      },
      'doubao-rest': {
        image: async (ctx) => this.generateImageWithDoubao(ctx.providerConfig, ctx.model, ctx.prompt, ctx.image, ctx.modeConfig, ctx.token),
        video: async (ctx) => this.generateVideoWithDoubao(ctx.providerConfig, ctx.model, ctx.prompt, ctx.images, ctx.imageMode, ctx.config.videoAudio, ctx.config.videoDuration, ctx.modeConfig, ctx.token)
      },
      'gemini-image-rest': {
        image: async (ctx) => this.generateGeminiImageRest(ctx.providerConfig, ctx.model, ctx.prompt, ctx.modeConfig, ctx.image, ctx.token)
      },
      'gemini-video-rest': {
        video: async (ctx) => this.generateGeminiVideo(ctx.providerConfig, ctx.model, ctx.prompt, ctx.images, ctx.config.videoAudio, ctx.config.videoDuration, ctx.modeConfig, ctx.token)
      }
    }
  }
  ...
  private async callOpenAIChatOrSearch(
    ...
    const url = providerConfig.responses ? resolveResponsesEndpointUrl(providerConfig, modeConfig) : resolveEndpointUrl(resolveBaseUrl(providerConfig, modeConfig), modeConfig.endpoint || 'chat/completions')
    const authMode = resolveAuthMode(getProviderProfile(providerConfig), modeConfig, providerConfig)

    const imageUrlPolicy = modeConfig.imageUrlPolicy ?? 'all'
    const safeImages = imageUrlPolicy === 'data-only' ? images.filter((part) => part.type === 'image_url' && !!parseDataUrl(part.image_url.url)) : images
    ...
    if (providerConfig.responses) {
      ...
      data = {
        model,
        input: inputContent.length > 0 ? [{ role: 'user', content: inputContent }] : question,
  ...
  private async callGeminiChatOrSearch(
    ...
    const root = getValueAtPath(response.data, ['response']) ?? getValueAtPath(response.data, ['data']) ?? response.data
    const candidate = getValueAtPath(root, ['candidates', 0])
    const cparts = getArrayAtPath(candidate, ['content', 'parts'])

    let text = ''
    const extractedImages: AIImage[] = []

    for (const p of cparts) {
      const partText = getStringAtPath(p, ['text'])
      if (partText) text += partText
      const inline = getValueAtPath(p, ['inlineData']) || getValueAtPath(p, ['inline_data'])
      const inlineData = getStringAtPath(inline, ['data'])
      if (inlineData) {
        extractedImages.push({
          data: Buffer.from(inlineData, 'base64'),
          mimeType: getStringAtPath(inline, ['mimeType']) || getStringAtPath(inline, ['mime_type']) || 'image/png'
        })
      }
    }
    ...
    const sources: Array<{ url: string; title?: string }> = []
    if (isSearch) {
      const groundingMetadata = getValueAtPath(candidate, ['groundingMetadata']) || getValueAtPath(candidate, ['grounding_metadata'])
      const groundingChunks = getArrayAtPath(groundingMetadata, ['groundingChunks']).length > 0 ? getArrayAtPath(groundingMetadata, ['groundingChunks']) : getArrayAtPath(groundingMetadata, ['grounding_chunks'])
      for (const chunk of groundingChunks) {
        const web = getValueAtPath(chunk, ['web']) || getValueAtPath(chunk, ['web_chunk'])
        const uri = getStringAtPath(web, ['uri'])
        if (uri) {
          sources.push({ url: uri, title: getStringAtPath(web, ['title']) })
        }
      }
    }
  ...
  private async generateImageWithDoubao(providerConfig: ProviderConfig, model: string, prompt: string, image: AIImage | undefined, modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIImage[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
  ...
  private async generateImageWithOpenAIRest(providerConfig: ProviderConfig, model: string, prompt: string, image: AIImage | undefined, modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIImage[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
  ...
  private async generateVideoWithOpenAIRest(providerConfig: ProviderConfig, model: string, prompt: string, images: AIContentPart[], modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIVideo[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
    ...
    let userContent: string | AIContentPart[] = content
  ...
  private buildDoubaoVideoContent(prompt: string, images: AIContentPart[], imageMode: VideoImageMode): UnknownRecord[] {
  ...
  private async generateGeminiImageRest(providerConfig: ProviderConfig, model: string, prompt: string, modeConfig: ProviderModeConfig, image?: AIImage, token?: AbortToken): Promise<AIImage[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
    ...
    if (model.includes('imagen')) {
      ...
      const predictions = getArrayAtPath(response.data, ['predictions'])
      const images: AIImage[] = []
      for (const p of predictions) {
        const bytes = getStringAtPath(p, ['bytesBase64Encoded'])
        if (bytes) {
          images.push({
            data: Buffer.from(bytes, 'base64'),
            mimeType: getStringAtPath(p, ['mimeType']) || 'image/png'
          })
        }
      }
    ...
    const root = getValueAtPath(response.data, ['response']) ?? getValueAtPath(response.data, ['data']) ?? response.data
    const candidates = getArrayAtPath(root, ['candidates'])
    const images: AIImage[] = []

    for (const c of candidates) {
      const cparts = getArrayAtPath(c, ['content', 'parts'])
      for (const p of cparts) {
        const inline = getValueAtPath(p, ['inlineData']) || getValueAtPath(p, ['inline_data'])
        const inlineData = getStringAtPath(inline, ['data'])
        if (inlineData) {
  ...
  private async generateGeminiVideo(providerConfig: ProviderConfig, model: string, prompt: string, images: AIContentPart[], videoAudio: boolean, videoDuration: number, modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIVideo[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
    ...
    if (directResult?.uri) {
      const download = await this.httpClient.request<ArrayBuffer>(
        {
    ...
    const operationName = getStringAtPath(response.data, ['name'])
    if (!operationName || typeof operationName !== 'string') {
      throw new Error('视频生成失败')
    }

    const baseOrigin = normalizeGeminiBaseUrl(providerConfig.url)
    const operation = await pollTask<unknown>(
      async (abortToken) => {
      ...
      (data): TaskPollResult<unknown> => {
        if (getValueAtPath(data, ['done']) !== true) {
          return { status: 'pending' }
        }
        if (getValueAtPath(data, ['error'])) {
          return {
            status: 'failed',
  ...
  private async generateVideoWithDoubao(providerConfig: ProviderConfig, model: string, prompt: string, images: AIContentPart[], imageMode: VideoImageMode, videoAudio: boolean, videoDuration: number, modeConfig: ProviderModeConfig, token?: AbortToken): Promise<AIVideo[]> {
    const baseUrl = resolveBaseUrl(providerConfig, modeConfig)
    ...
    const taskId = getStringAtPath(response.data, ['task_id']) || getStringAtPath(response.data, ['data', 'task_id']) || getStringAtPath(response.data, ['data', 'id']) || getStringAtPath(response.data, ['id'])
    ...
    const videoUrl = await pollTask<string>(
      ...
      (data): TaskPollResult<string> => {
        const statusRaw = getStringAtPath(data, ['status']) || getStringAtPath(data, ['data', 'status'])
        if (statusRaw === 'failed') {
  ...
  async callAI(question: string, images: AIContentPart[] = [], token?: AbortToken): Promise<{ text: string; images: AIImage[] }> {
  ...
  async generateVideo(prompt: string, images: AIContentPart[], imageMode: VideoImageMode = 'auto', token?: AbortToken): Promise<AIVideo[]> {
...
abstract class BaseFeatureHandler implements FeatureHandler {
  ...
  abstract execute(msg: MessageContext, args: string[]): Promise<void>

  constructor(protected readonly configManagerPromise: Promise<ConfigManager>) {}
...
type BooleanSetting = {
  command: string
  key: 'imageSaveToFavorites' | 'imagePreview' | 'videoSaveToFavorites' | 'videoPreview' | 'videoAudio'
  statusTitle: string
  successText: string
}

const handleBooleanSetting = async (msg: MessageContext, args: string[], feature: 'image' | 'video', configManager: ConfigManager, config: DB, settings: BooleanSetting[]): Promise<boolean> => {
  const setting = settings.find(({ command }) => command === args[1]?.toLowerCase())
  if (!setting) return false

  const usage = buildCommandUsage(`aix ${feature} ${setting.command} [on|off]`)
  const state = args[2]?.toLowerCase()
  if (!state) {
    requireUser(args.length === 2, usage)
    await MessageSender.sendOrEdit(msg, `${setting.statusTitle}\n\n📄 当前状态: ${config[setting.key] ? '开启' : '关闭'}`, {})
    return true
  }
  ...
  await MessageSender.sendOrEdit(msg, `✅ ${setting.successText}已${enabled ? '开启' : '关闭'}`, {})
  return true
}

const handlePromptOptimizationSetting = async (msg: MessageContext, args: string[], feature: 'image' | 'video', configManager: ConfigManager, config: DB): Promise<boolean> => {
  if (args[1]?.toLowerCase() !== 'optimize') return false

  const isImage = feature === 'image'
  const label = isImage ? '图片' : '视频'
  const labels = isImage ? IMAGE_PROMPT_LENGTH_LABELS : VIDEO_PROMPT_LENGTH_LABELS
  const enabled = isImage ? config.promptOptimize !== false : config.videoPromptOptimize !== false
  const mode = normalizePromptLengthMode(isImage ? config.promptLength : config.videoPromptLength)
  const command = parsePromptOptimizationCommand(args, feature)

  if (command.type === 'status') {
    await MessageSender.sendOrEdit(msg, `🧠 <b>${label}提示词优化:</b>\n\n` + `状态: ${enabled ? '开启' : '关闭'}\n` + `长度: <code>${mode}</code>（${labels[mode]}）`, {})
  } else if (command.type === 'toggle') {
    await configManager.updateConfig((cfg) => {
      if (isImage) cfg.promptOptimize = command.enabled
      else cfg.videoPromptOptimize = command.enabled
    })
    await MessageSender.sendOrEdit(msg, `✅ ${label}提示词优化已${command.enabled ? '开启' : '关闭'}`, {})
  } else if (command.type === 'length-status') {
    await MessageSender.sendOrEdit(msg, `📏 ${label}优化提示词长度: <code>${mode}</code>（${labels[mode]}）`, {})
  } else {
    await configManager.updateConfig((cfg) => {
      if (isImage) cfg.promptLength = command.mode
      else cfg.videoPromptLength = command.mode
    })
    await MessageSender.sendOrEdit(msg, `✅ ${label}优化提示词长度已设为 ${command.mode}（${labels[command.mode]}）`, {})
  }
  return true
}
...
class ConfigFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    const configManager = await this.getConfigManager()
    const config = configManager.getConfig()

    if (args.length < 2 || (args.length === 2 && args[1].toLowerCase() === 'list')) {
      const list =
        Object.values(config.configs)
          ...
          .join('\n') || '暂无配置'
      await this.editMessage(msg, `📋 <b>API 配置列表:</b>\n\n⚙️ 配置:\n${list}`)
      return
    }
    ...
    if (action === 'stream' || action === 'responses') {
      ...
      await this.setProviderBooleanOption(msg, args, configManager, action)
      return
    }
    if (action === 'type') {
      requireUser(args.length >= 4, '参数不足')
  ...
  private parseAddConfigArgs(args: string[]): {
    ...
    requireUser(!!tag && (tail.length === 1 || tail.length === 2), '参数格式错误')
  ...
  private async addConfig(msg: MessageContext, args: string[], configManager: ConfigManager): Promise<void> {
    ...
    requireUser(meId !== undefined && String(msg.chat.id) === meId, '出于安全考虑，禁止在公开场景添加/修改 API 密钥')
  ...
  private async setProviderBooleanOption(msg: MessageContext, args: string[], configManager: ConfigManager, option: 'stream' | 'responses'): Promise<void> {
    ...
    const settingText = option === 'stream' ? 'Stream 设置为' : 'Responses(chat/search) 模式设置为'
    await this.editMessage(msg, `✅ 已将配置 <code>${tag}</code> 的 ${settingText} <code>${enabled ? 'on' : 'off'}</code>`)
  }

  private async setProviderType(msg: MessageContext, args: string[], configManager: ConfigManager): Promise<void> {
    ...
    await this.editMessage(msg, `✅ 已将配置 <code>${tag}</code> 的 Type 设置为 <code>${type}</code>`)
  }
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (args.length < 2) {
      const details = PROVIDER_MODES.map((mode) => {
        ...
        return `${tagIcon} ${mode} 配置: <code>${config[tagKey] || '未设置'}</code>\n` + `${modelIcon} ${mode} 模型: <code>${config[modelKey] || '未设置'}</code>`
      }).join('\n')
      await this.editMessage(msg, `🤖 <b>当前 AI 配置:</b>\n\n${details}`)
      return
    }
    ...
    await this.editMessage(msg, `✅ ${mode} 模型 已切换到:\n\n🏷️ 配置: <code>${tag}</code>\n🧠 模型: <code>${model}</code>`)
  }
}
...
class CollapseFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (args.length < 2) {
      await this.editMessage(msg, `📖 <b>消息折叠状态:</b>\n\n📄 当前状态: ${config.collapse ? '开启' : '关闭'}`)
      return
    }

    const state = args[1].toLowerCase()
    requireUser(args.length === 2, buildCommandUsage('aix collapse on|off'))
    requireUser(state === 'on' || state === 'off', '参数必须是 on 或 off')
...
class TelegraphFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (action === 'on' || action === 'off') {
      requireUser(args.length === 2, buildCommandUsage(`aix telegraph ${action}`))
      const enabled = action === 'on'
      await configManager.updateConfig((cfg) => {
        cfg.telegraph.enabled = enabled
      })
      await this.editMessage(msg, `✅ Telegraph 已${enabled ? '开启' : '关闭'}`)
      return
    }
    if (action === 'limit') {
      requireUser(args.length === 3, buildCommandUsage('aix telegraph limit <正整数>'))
      await this.setTelegraphLimit(msg, args, configManager)
      return
    }
    if (action === 'del') {
      requireUser(args.length === 3, buildCommandUsage('aix telegraph del <序号|all>'))
      await this.deleteTelegraphItem(msg, args, configManager)
      return
    }
    throw new UserError(buildCommandUsage('aix telegraph [on|off|limit <正整数>|del <序号|all>]'))
  }

  private async showTelegraphStatus(msg: MessageContext, config: DB): Promise<void> {
    const entries = config.telegraph.list.map((item, index) => `${index + 1}. <a href="${item.url}">🔗 ${item.title}</a>\n`).join('')
    const status = `📰 <b>Telegraph 状态:</b>\n\n` + `🌐 当前状态: ${config.telegraph.enabled ? '开启' : '关闭'}\n` + `📊 限制数量: <code>${config.telegraph.limit}</code>\n` + `📈 记录数量: <code>${config.telegraph.list.length}/${config.telegraph.limit}</code>` + (entries ? `\n\n${entries}` : '')
  ...
  private async deleteTelegraphItem(msg: MessageContext, args: string[], configManager: ConfigManager): Promise<void> {
    ...
    requireUser(!Number.isNaN(idx) && idx >= 0 && idx < config.telegraph.list.length, `序号超出范围 (1-${config.telegraph.list.length})`)
...
class TimeoutFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (args.length < 2) {
      await this.editMessage(msg, `⏱️ <b>当前超时设置:</b>\n\n⏰ 超时时间: <code>${config.timeout} 秒</code>`)
      return
    }

    requireUser(args.length === 2, buildCommandUsage('aix timeout <整数秒数 1-1800>'))
    const timeout = parseStrictInteger(args[1])
    requireUser(timeout !== null && timeout >= 1 && timeout <= 1800, '超时时间必须是 1 到 1800 之间的整数')
...
class QuestionFeature extends BaseFeatureHandler {
  ...
  private getOperationScope(msg: MessageContext): string {
    return String(msg.chat.id)
  }

  cancelCurrentOperation(reason: string = '操作已取消', msg?: MessageContext): boolean {
    if (msg) {
  ...
  private async runQuestion(msg: MessageContext, question: string): Promise<void> {
  ...
  async askFromReply(msg: MessageContext): Promise<void> {
  ...
  async handleQuestion(msg: MessageContext, question: string, token?: AbortToken): Promise<void> {
    ...
    const cleanedQuestion = avatarQuestion ? questionText.replace(/^(?:a|avatar|头像|頭像)\s*/i, '').trim() : questionText
    const [replyImageParts, messageImageParts] = await Promise.all([getMessageImageParts(replyMsg), getMessageImageParts(msg)])
    ...
    if (config.telegraph.enabled && formattedAnswer.length > 4050) {
      await this.handleLongContentWithTelegraph(msg, finalQuestion, answer, replyToId, token)
    } else {
  ...
  private async handleLongContentWithTelegraph(msg: MessageContext, question: string, rawAnswer: string, replyToId?: number, token?: AbortToken): Promise<void> {
    const configManager = await this.getConfigManager()
    const config = configManager.getConfig()
    ...
    await MessageSender.sendNew(msg, questionBlock + answerBlock, { disableWebPreview: true }, replyToId)
...
class SearchFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    try {
      ...
      const effectivePrompt = promptInput || replyText || '请分析图片并搜索相关信息'
      ...
      const collapseSafe = config.collapse
      const htmlAnswer = TelegramFormatter.markdownToHtml(text || 'AI 回复为空', { collapseSafe })
      ...
      if (config.telegraph.enabled && formatted.length > 4050) {
        ...
        const telegraphResult = await this.messageUtils.createTelegraphPage(telegraphMarkdown, effectivePrompt, token)

        const poweredByText = `\n<i>🍀Powered by ${config.currentSearchTag}</i>`
        const qBlock = config.collapse ? `Q:\n<blockquote expandable>${safeQuestion}</blockquote>\n` : `Q:\n${safeQuestion}\n`
        const aBlock = config.collapse ? `A:\n<blockquote expandable>📰内容较长，Telegraph 观感更好：\n🔗 <a href="${telegraphResult.url}">点我阅读内容</a></blockquote>${poweredByText}` : `A:\n📰内容较长，Telegraph 观感更好：\n🔗 <a href="${telegraphResult.url}">点我阅读内容</a>${poweredByText}`

        await MessageSender.sendNew(msg, qBlock + aBlock, { disableWebPreview: true }, replyToId)
      ...
      if (/policy|safety|moderation|content_filter|不合规|违规|敏感|拒绝|安全策略|blocked/i.test(raw)) {
        await MessageSender.sendOrEdit(
          msg,
          `⚠️ 当前搜索内容可能不合规或被上游拦截，换个说法再试。

<blockquote expandable>${htmlEscape(raw)}</blockquote>`,
          {}
        )
      } else {
...
const explicitlyRequestsImageCollage = (prompt: string): boolean => /(?:[二两三四五六七八九十\d]+宫格|宫格图|拼成.{0,8}(?:宫格|一张图|一张图片)|组成.{0,8}(?:宫格|一张图|一张图片)|图片.{0,6}(?:拼接|拼在一起|合在一起|合成一张)|(?:拼接|拼图|合并).{0,8}图片)/.test(prompt)
...
const HIDDEN_IMAGE_VARIATION_DIRECTIONS = ['正面近景、自然互动动作、明亮暖色层次', '侧面中景、富有张力的动作、清爽冷色层次', '动态斜构图、夸张表情、强烈明暗对比', '俯视或高机位、轻松姿态、通透明快配色', '全身构图、鲜明肢体语言、柔和细腻质感', '低机位特写、戏剧性动作、浓郁电影色彩']
...
class PartialImageBatchTimeoutError extends Error {
  constructor(readonly completedSlots: Array<{ index: number; image: AIImage }>) {
    super('请求超时')
    this.name = 'PartialImageBatchTimeoutError'
  }
}

const generateImageBatchAllOrTimeout = async (imageCount: number, token: AbortToken, generateSlot: (index: number) => Promise<AIImage[]>): Promise<AIImage[]> => {
  ...
  try {
    while (slots.some((image) => !image)) {
      token.throwIfAborted()
      const pendingIndices = slots.map((image, index) => (image ? -1 : index)).filter((index) => index >= 0)
      await runOrderedJobs(pendingIndices.length, IMAGE_GENERATION_CONCURRENCY, token, async (pendingIndex) => {
        ...
        try {
          ...
          if (token.aborted) throw error
          if (!isRetryableError(error) || attempts[slotIndex] >= MAX_IMAGE_GENERATION_ATTEMPTS) {
            throw error
          }
        }
      })
    ...
    if (token.aborted) {
      const completedSlots = slots.map((image, index) => (image ? { index, image } : undefined)).filter((slot): slot is { index: number; image: AIImage } => !!slot)
...
const parseMediaCountRequest = (args: string[], startIndex: number): { prompt: string; count: number } => {
  ...
  for (let i = 0; i < rawTokens.length; i++) {
    ...
    if (low === '--n' || low === '-n' || low === '--count') {
      requireUser(!!rawTokens[i + 1], buildCommandUsage(`aix video --n <1-${MAX_VIDEO_GENERATION_COUNT}> [提示词]`))
...
const consumeAvatarLockArg = (args: string[]): { args: string[]; forceAvatar: boolean } => {
...
class ImageFeature extends BaseFeatureHandler {
  ...
  private parseImageRequest(args: string[]): { prompt: string; originalPrompt: string; count: number; perImagePrompts: string[] } {
    ...
    const resolvedCount = natural.perImagePrompts.length > 0 ? natural.perImagePrompts.length : natural.count || 1
    return {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (await handlePromptOptimizationSetting(msg, args, 'image', configManager, config)) return

    if (
      await handleBooleanSetting(msg, args, 'image', configManager, config, [
        {
          command: 'save',
    ...
    const wantsAvatar = forceAvatar || ['a', 'avatar', '头像', '頭像'].includes(args[1]?.toLowerCase() || '')
    ...
    let imageCount = localParsedRequest.perImagePrompts.length > 0 ? localParsedRequest.perImagePrompts.length : localParsedRequest.count || imageRequest.count
    requireUser(imageCount <= MAX_IMAGE_GENERATION_COUNT, `单次最多生成 ${MAX_IMAGE_GENERATION_COUNT} 张图片`)
    ...
    try {
      ...
      if (wantsAvatar) {
        ...
        if (avatarPart) {
          ...
          throw new UserError('无法获取被回复用户的头像，请确认对方有公开头像')
        }
      }
      ...
      if (referenceCount > 0) {
        const statusText = optimizePrompt ? `✅ 已识别 ${referenceCount} 张图片` : `✅ 已识别 ${referenceCount} 张图片，正在结合生成...`
        if (messageImageParts.length > 0) {
          generationStatusMsg = await MessageSender.sendNew(msg, statusText)
          await deleteMessageOrGroup(msg)
        } else {
          generationStatusMsg = await MessageSender.sendOrEdit(msg, statusText, {})
        }
      ...
      if (optimizePrompt) {
        requireUser(!!config.currentChatTag && !!config.currentChatModel && !!config.configs[config.currentChatTag], '⚠️ 图片提示词优化已开启，请先配置聊天模型')
        generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, buildPromptOptimizationStatusText(originalPrompt))
        let analyzedPrompts: string[] = []
        try {
          const analyzed = await runAIWithTimeout(this.aiService, buildImageRequirementAnalysisRequest(originalPrompt, config.promptLength), imageParts, token, Math.max(Math.min(config.timeout || 30, 45), 20) * 1000)
        ...
        if (analyzedPrompts.length > 0) {
          ...
          requireUser(imageCount <= MAX_IMAGE_GENERATION_COUNT, `分析结果要求生成 ${imageCount} 张图片，单次上限为 ${MAX_IMAGE_GENERATION_COUNT} 张`)
          ...
          generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, imageCount > 1 ? buildMultiPromptStatusText('提示词已分别优化，正在生图...', analyzedPrompts) : buildOptimizedPromptStatusText(prompt))
        } else if (imageCount > 1 && explicitPerImagePrompts.length === imageCount) {
          // 逐张内容优先于 AI 自动规划；开启优化时也只分别润色，不改变对应关系。
          const optimizedPrompts = await runOrderedJobs(explicitPerImagePrompts.length, IMAGE_GENERATION_CONCURRENCY, token, async (index) => {
            const explicitPrompt = explicitPerImagePrompts[index]
            try {
              const one = await runAIWithTimeout(this.aiService, buildImagePromptOptimizationRequest(explicitPrompt, config.promptLength), imageParts, token, Math.max(config.timeout || 30, 20) * 1000)
              return sanitizeOptimizedImagePrompt(one.text, explicitPrompt)
            } catch (error) {
              console.warn('[aix] 逐张提示词优化失败，使用原提示词:', error)
          ...
          generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, buildMultiPromptStatusText('逐张提示词已优化，正在生图...', optimizedPrompts))
        } else if (imageCount > 1) {
          const optimized = await runAIWithTimeout(this.aiService, buildMultiImagePromptPlanRequest(originalPrompt, imageCount, config.promptLength), imageParts, token, Math.max(config.timeout || 30, 20) * 1000)
          ...
          if (optimizedPrompts.length !== imageCount) {
            ...
            if (needsExplicitStickerTextPrompt(originalPrompt)) {
              ...
              optimizedPrompts = await runOrderedJobs(imageCount, IMAGE_GENERATION_CONCURRENCY, token, async (i) => {
                const fallbackPrompt = `${originalPrompt}，第 ${i + 1} 张采用不同表情、动作、构图、色彩和氛围。`
                try {
                  const one = await runAIWithTimeout(this.aiService, buildImagePromptOptimizationRequest(fallbackPrompt, config.promptLength), imageParts, token, Math.max(config.timeout || 30, 20) * 1000)
                  return sanitizeOptimizedImagePrompt(one.text, fallbackPrompt)
                } catch (error) {
          ...
          prompt = optimizedPrompts[0] || originalPrompt
          optimizedPromptText = optimizedPrompts.join('\n\n')
          generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, buildMultiPromptStatusText('提示词已分别优化，正在生图...', optimizedPrompts))
        } else {
          const optimized = await runAIWithTimeout(this.aiService, buildImagePromptOptimizationRequest(originalPrompt, config.promptLength), imageParts, token, Math.max(config.timeout || 30, 20) * 1000)
          ...
          generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, buildOptimizedPromptStatusText(prompt))
        }
        if (!imageGenerationDone) {
          ...
          imageStatusTransitionId = setTimeout(() => {
            ...
            void replaceStatusMessage(currentStatus, msg, PROCESSING_TEXT.image)
              .then(async (statusMsg) => {
                if (imageGenerationDone || generationStatusMsg !== currentStatus) {
                  await deleteStatusMessage(statusMsg)
                  return
                }
      ...
      const normalizedPromptForImage = imageCount > 1 ? normalizeSingleImagePrompt(prompt, 0, imageCount) : prompt
      // 本地规则只在执行层静默移除数量控制词；关闭优化时，展示层始终保留用户完整原话。
      const displayPrompts = optimizePrompt ? (perImagePrompts.length > 0 ? perImagePrompts : Array.from({ length: imageCount }, (_, i) => normalizeSingleImagePrompt(prompt, i, imageCount))) : Array.from({ length: imageCount }, () => originalPrompt)
      const imageCaption = imageCount > 1 ? buildGeneratedPromptListText(displayPrompts) : optimizePrompt ? optimizedPromptText || normalizedPromptForImage : originalPrompt
      ...
      let images: AIImage[] = []
      const executionPrompts = perImagePrompts.length > 0 ? perImagePrompts : Array.from({ length: imageCount }, (_, i) => normalizeSingleImagePrompt(prompt, i, imageCount))
      try {
        let inputImage: AIImage | undefined
        if (imageParts.length > 0) {
          inputImage = (await resolveMergedImageParts(imageParts, this.httpClient, token)) || undefined
        ...
        images = await generateImageBatchAllOrTimeout(imageCount, token, async (i) => {
          ...
          return inputImage ? await this.aiService.editImage(singlePrompt, inputImage, token) : await this.aiService.generateImage(singlePrompt, token)
        })
      } catch (error) {
        if (error instanceof PartialImageBatchTimeoutError && error.completedSlots.length > 0) {
          ...
          const partialPrompts = optimizePrompt ? completed.map((slot) => displayPrompts[slot.index]) : originalPrompt
      ...
      await this.messageUtils.sendImages(msg, imageCount > 1 ? images : images.slice(0, 1), imageCount > 1 ? (optimizePrompt ? displayPrompts : originalPrompt) : imageCaption, replyToId, token)
      await deleteMessageOrGroup(msg)
    } finally {
      imageGenerationDone = true
      if (imagePhaseTimeoutId) clearTimeout(imagePhaseTimeoutId)
      if (imageStatusTransitionId) clearTimeout(imageStatusTransitionId)
      await deleteStatusMessage(generationStatusMsg)
      this.aiService.releaseToken(token)
    }
  }
}
...
class VideoFeature extends BaseFeatureHandler {
  ...
  async execute(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (await handlePromptOptimizationSetting(msg, args, 'video', configManager, config)) return

    let imageMode: VideoImageMode = 'auto'
    let promptStartIndex = 1
    if (
      await handleBooleanSetting(msg, args, 'video', configManager, config, [
        {
          command: 'save',
    ...
    if (subCommand === 'duration') {
      if (!args[2]) {
        requireUser(args.length === 2, buildCommandUsage('aix video duration [5-20]'))
        await this.editMessage(msg, `⏱️ <b>视频时长:</b>\n\n⏰ 当前时长: <code>${config.videoDuration} 秒</code>`)
        return
      }
      requireUser(args.length === 3, buildCommandUsage('aix video duration [5-20]'))
      const duration = parseStrictInteger(args[2])
      requireUser(duration !== null && duration >= 5 && duration <= 20, '时长必须是 5-20 的整数')
      await configManager.updateConfig((cfg) => {
    ...
    requireUser(videoCount <= MAX_VIDEO_GENERATION_COUNT, `单次最多生成 ${MAX_VIDEO_GENERATION_COUNT} 个视频`)
    ...
    try {
      let [replyImageParts, messageImageParts] = await Promise.all([getMessageImageParts(replyMsg), getMessageImageParts(msg)])
      token.throwIfAborted()
      if (forceAvatar) {
        replyImageParts = []
        const avatarPart = await getAvatarImagePartFromMessageSender(replyMsg)
      ...
      if (imageMode === 'first') {
        requireUser(allImageParts.length >= 1, 'video first 需要至少 1 张首帧图片')
      }
      if (imageMode === 'firstlast') {
        requireUser(allImageParts.length >= 2, 'video firstlast 需要至少 2 张首尾帧图片')
      }
      requireUser(hasPrompt || allImageParts.length > 0, '至少需要提示词或参考图片')
      ...
      if (config.videoPromptOptimize !== false && finalPrompt.trim()) {
        requireUser(!!config.currentChatTag && !!config.currentChatModel && !!config.configs[config.currentChatTag], '⚠️ 视频提示词优化已开启，请先配置聊天模型')
        if (videoCount > 1) {
          optimizedVideoPrompts = await runOrderedJobs(videoCount, VIDEO_GENERATION_CONCURRENCY, token, async (i) => {
            ...
            const optimized = await runAIWithTimeout(this.aiService, buildVideoPromptOptimizationRequest(basePrompt, config.videoPromptLength), allImageParts, token, Math.max(config.timeout || 30, 20) * 1000)
            if (!optimized.text?.trim() || optimized.text.trim() === 'AI 回复为空') {
              optimized.text = basePrompt
            }
            return sanitizeOptimizedImagePrompt(optimized.text, basePrompt)
          })
          finalPrompt = optimizedVideoPrompts[0] || finalPrompt
          generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, buildMultiPromptStatusText('视频提示词已分别优化，正在生成视频...', optimizedVideoPrompts))
        } else {
          const optimized = await runAIWithTimeout(this.aiService, buildVideoPromptOptimizationRequest(finalPrompt, config.videoPromptLength), allImageParts, token, Math.max(config.timeout || 30, 20) * 1000)
          if (!optimized.text?.trim() || optimized.text.trim() === 'AI 回复为空') {
        ...
        generationStatusMsg = await replaceStatusMessage(generationStatusMsg, msg, PROCESSING_TEXT.video)
      }
      ...
      const normalizedVideoPrompts = videoCount > 1 ? (optimizedVideoPrompts.length > 0 ? optimizedVideoPrompts : Array.from({ length: videoCount }, (_, i) => normalizeSingleImagePrompt(finalPrompt, i, videoCount))) : [finalPrompt]
      const videoBatches = await runOrderedJobs(normalizedVideoPrompts.length, VIDEO_GENERATION_CONCURRENCY, token, (i) => this.aiService.generateVideo(normalizedVideoPrompts[i], imageParts, imageMode, token))
      const videos = videoBatches.flat()
      if (videos.length === 0) throw new Error('AI 回复为空')
      const videoCaption = videoCount > 1 ? buildGeneratedPromptListText(normalizedVideoPrompts) : finalPrompt
      await this.messageUtils.sendVideos(msg, videos, videoCaption, replyToId, token)
      await deleteMessageOrGroup(msg)
    } finally {
      await deleteStatusMessage(generationStatusMsg)
...
class AIXPlugin extends Plugin {
  ...
  constructor() {
    ...
    const messageUtils = new MessageUtils(this.configManagerPromise, this.httpClient)
    this.featureRegistry = new FeatureRegistry()
    this.questionFeature = new QuestionFeature(this.aiService, this.configManagerPromise, messageUtils)
    this.imageFeature = new ImageFeature(this.aiService, this.configManagerPromise, this.httpClient, messageUtils)
    this.registerFeatures(messageUtils)
  }

  private registerFeatures(messageUtils: MessageUtils): void {
    ;[new ConfigFeature(this.configManagerPromise), new ModelFeature(this.configManagerPromise), new PromptFeature(this.configManagerPromise), new CollapseFeature(this.configManagerPromise), new TelegraphFeature(this.configManagerPromise), new TimeoutFeature(this.configManagerPromise)].forEach(
      (feature) => {
        this.featureRegistry.register(feature)
      }
    )

    this.featureRegistry.register(new SearchFeature(this.aiService, this.configManagerPromise, messageUtils))
    this.featureRegistry.register(this.imageFeature)
    this.featureRegistry.register(new VideoFeature(this.aiService, this.configManagerPromise, messageUtils))
  }
...
• <code>${mainPrefix}aix stop</code> - 取消当前插件内所有 AIX 任务`
  ...
  cmdHandlers: Record<string, (msg: MessageContext, trigger?: MessageContext) => Promise<void>> = {
    aix: async (msg: MessageContext, trigger?: MessageContext) => {
      try {
        ...
        if (sub === 'stop') {
          ...
          await MessageSender.sendOrEdit(trigger || msg, cancelledQuestion || cancelledOperations > 0 ? '🚫 操作已取消' : '⚠️ 当前没有正在执行的任务')
          return
        }
        if (sub === 'input') {
          if (args.length === 1) {
