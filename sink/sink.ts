...
const getRepliedContentUrl = async (msg: MessageContext): Promise<string> => {
  const replied = await safeGetReplyMessage(msg)
  if (!replied) return ''

  const text = replied.text
  for (const entity of replied.entities) {
    if (entity.is('text_link')) return entity.params.url
    if (entity.is('url')) return entity.text
  }

  const plainUrl = text.match(/https?:\/\/[^\s<>"']+/i)?.[0]
  if (plainUrl) return plainUrl.replace(/[),.;!?，。；！？）]+$/, '')

  if (replied.media?.type === 'webpage') return replied.media.preview.url
...
class SinkPlugin extends Plugin {
  ...
  private readonly deletionTimers = new Set<ReturnType<typeof setTimeout>>()
  ...
  private scheduleMessageDeletion(msg: MessageContext): void {
  ...
  private async handleConfig(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (action === 'show') {
      ...
      await msg.edit({
        text: html(
          ['<b>⚙️ Sink 配置</b>', `<b>实例：</b> ${config.baseUrl ? `<code>${htmlEscape(config.baseUrl)}</code>` : '未设置'}`, `<b>Token：</b> <code>${htmlEscape(maskToken(config.siteToken))}</code>`, `<b>创建结果 10 秒后删除：</b> ${config.autoDeleteCreatedMessage ? '已开启' : '已关闭'}`].join('\n')
        )
      })
      return
    }
    ...
    if (action === 'url') {
      ...
      await msg.edit({
        text: html(`✅ Sink 实例已设置为 <code>${htmlEscape(baseUrl)}</code>`)
      })
      return
    }
  ...
  private async handleVerify(msg: MessageContext): Promise<void> {
    ...
    await msg.edit({
      text: html(`✅ Sink API 认证成功\n<code>${htmlEscape(config.baseUrl)}</code>`)
    })
  }

  private async handleCreate(msg: MessageContext, args: string[]): Promise<void> {
    ...
    await msg.edit({
      text: html(`${formatted ? `✅ <b>短链已创建</b>\n\n${formatted}` : '✅ 短链已创建'}${deletionNotice}`),
      disableWebPreview: true
    })
    if (deleteAfterCreate) this.scheduleMessageDeletion(msg)
  }

  private async handleGet(msg: MessageContext, slug: string): Promise<void> {
    ...
    await msg.edit({
      text: html(formatted ? `🔎 <b>短链详情</b>\n\n${formatted}` : '未找到该短链'),
      disableWebPreview: true
    })
  }

  private async handleSearch(msg: MessageContext, query: string): Promise<void> {
    ...
    await msg.edit({
      text: html(`🔎 <b>搜索结果（${links.length}）</b>\n\n${items.join('\n\n')}`),
      disableWebPreview: true
    })
  }

  private async handleList(msg: MessageContext): Promise<void> {
    const { client, config } = await this.getClient()
    ...
    await msg.edit({
      text: html(renderChunk(chunks[0], 0)),
      disableWebPreview: true
    })
    for (let index = 1; index < chunks.length; index += 1) {
      await msg.replyText(html(renderChunk(chunks[index], index)), {
        disableWebPreview: true
      })
    }
  }

  private async handleDelete(msg: MessageContext, inputSlugs: string[]): Promise<void> {
  ...
  cmdHandlers: Record<string, (msg: MessageContext) => Promise<void>> = {
    sink: async (msg: MessageContext) => {
      try {
        const text = msg.text.trim()
        const args = tokenizeArguments(text).slice(1)
        const subcommand = args[0]?.toLowerCase()

        if (!subcommand && msg.replyToMessage) {
        ...
        } else if (/^https?:\/\//i.test(args[0]) || msg.replyToMessage) {
        ...
        await msg.edit({
          text: html(`❌ ${htmlEscape(friendlyError(error))}`),
          disableWebPreview: true
        })
      }
    }
