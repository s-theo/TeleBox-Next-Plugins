...
```text
.cd2 config endpoint http://host:19798
.cd2 config account YOUR_CLOUDDRIVE_USERNAME YOUR_CLOUDDRIVE_PASSWORD
.cd2 login
.cd2 verify
```
...
```text
.cd2 config token YOUR_CD2_API_TOKEN
```

WebDAV management and Telegram upload:

```text
.cd2 config dav-url http://host:19798/dav
.cd2 dav on
.cd2 dav account on /Telegram
.cd2 config dav-user USER PASSWORD
.cd2 config dav-root /Telegram
.cd2 dav status
.cd2 dav ls /
