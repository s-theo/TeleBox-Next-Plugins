...
<b>5. 云 API 管理</b>
• <code>${commandName} cloud list</code> 查看云 API
• <code>${commandName} cloud add webdav URL USER PASSWORD</code>
• <code>${commandName} cloud add local /本地目录</code>
• <code>${commandName} cloud add pikpak USER PASSWORD</code>
• <code>${commandName} cloud add aliyun-refresh REFRESH_TOKEN [openapi]</code>
• <code>${commandName} cloud add aliyun-oauth REFRESH ACCESS EXPIRES</code>
• <code>${commandName} cloud add baidu-oauth REFRESH ACCESS EXPIRES</code>
• <code>${commandName} cloud add onedrive-oauth REFRESH ACCESS EXPIRES</code>
• <code>${commandName} cloud add google-oauth REFRESH ACCESS EXPIRES</code>
• <code>${commandName} cloud add google-refresh CLIENT SECRET REFRESH</code>
• <code>${commandName} cloud add xunlei-oauth REFRESH ACCESS EXPIRES</code>
• <code>${commandName} cloud remove CLOUD USER confirm</code>
• <code>${commandName} cloud config get|set CLOUD USER KEY VALUE</code>
...
• <code>${commandName} config token YOUR_CD2_API_TOKEN</code>
...
• <code>${commandName} config account USER PASSWORD</code>
• <code>${commandName} config path /</code>
• <code>${commandName} config dav-url http://host:19798/dav</code>
• <code>${commandName} config dav-user USER PASSWORD</code>
• <code>${commandName} config dav-root /Telegram</code>
...
const davRequest = (config: Cd2Config, method: string, remotePath: string, body?: Buffer, extraHeaders: Record<string, string> = {}): Promise<WebDavResponse> => {
  ...
  if (!config.webdavUrl || !username || !password) throw new Error(`请先配置 WebDAV 地址和账号，使用 ${commandName} config dav-url|dav-user 或 ${commandName} config account|login`)
...
class Cd2Client {
  ...
  private async request(method: string, request: Record<string, unknown>, streaming: boolean): Promise<RpcResponse[]> {
    ...
    if (!config.endpoint) throw new Error(`请先配置 CD2 地址，使用 ${commandName} config endpoint ...`)
...
class Cd2Plugin extends Plugin {
  ...
  private async handleConfig(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (action === 'endpoint' || action === 'url') {
      if (!args[1]) throw new Error(`用法：${commandName} config endpoint http://host:19798`)
    ...
    if (action === 'account') {
      if (!args[1] || !args[2]) throw new Error(`用法：${commandName} config account USER PASSWORD`)
    ...
    if (action === 'path') {
      if (!args[1]) throw new Error(`用法：${commandName} config path /目录`)
    ...
    if (action === 'dav-url') {
      if (!args[1]) throw new Error(`用法：${commandName} config dav-url http://host:19798/dav`)
    ...
    if (action === 'dav-user') {
      if (!args[1] || !args[2]) throw new Error(`用法：${commandName} config dav-user USER PASSWORD`)
      await this.updateConfig({ webdavUsername: args[1], webdavPassword: args[2] })
      await msg.edit({ text: '✅ WebDAV 账号已保存' })
      return
    }
    if (action === 'dav-root') {
      if (!args[1]) throw new Error(`用法：${commandName} config dav-root /目录`)
    ...
    throw new Error(`用法：${commandName} config endpoint|token|account|path|dav-url|dav-user|dav-root|show`)
  }

  private async handleLogin(msg: MessageContext): Promise<void> {
    const config = await this.getConfig()
    if (!config.accountUsername || !config.accountPassword) throw new Error(`请先配置：${commandName} config account USER PASSWORD`)
  ...
  private async handleSearch(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (!keyword) throw new Error(`用法：${commandName} search 关键词 [路径]`)
    const config = await this.getConfig()
  ...
  private async handleCloud(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (action === 'add') {
      ...
      if (type === 'webdav') {
        if (!args[2] || !args[3] || !args[4]) throw new Error(`用法：${commandName} cloud add webdav URL USER PASSWORD`)
        method = 'APILoginWebDav'
        request = { serverUrl: args[2], userName: args[3], password: args[4] }
      } else if (type === 'local') {
        if (!args[2]) throw new Error(`用法：${commandName} cloud add local /本地目录`)
        method = 'APIAddLocalFolder'
        request = { localFolderPath: args[2] }
      } else if (type === 'pikpak') {
        if (!args[2] || !args[3]) throw new Error(`用法：${commandName} cloud add pikpak USER PASSWORD`)
        method = 'APILoginPikPak'
        request = { userName: args[2], password: args[3] }
      } else if (type === '115-cookie') {
        if (!args[2]) throw new Error(`用法：${commandName} cloud add 115-cookie COOKIE`)
        method = 'APILogin115Editthiscookie'
        ...
        if (!args[2] || !args[3] || !args[4]) throw new Error(`用法：${commandName} cloud add ${type} REFRESH_TOKEN ACCESS_TOKEN EXPIRES_IN`)
        ...
        if (!args[2]) throw new Error(`用法：${commandName} cloud add aliyun-refresh REFRESH_TOKEN [openapi]`)
        method = 'APILoginAliyundriveRefreshtoken'
        request = { refreshToken: args[2], useOpenAPI: args[3]?.toLowerCase() === 'openapi' }
      } else if (type === 'google-refresh') {
        if (!args[2] || !args[3] || !args[4]) throw new Error(`用法：${commandName} cloud add google-refresh CLIENT_ID CLIENT_SECRET REFRESH_TOKEN`)
        method = 'ApiLoginGoogleDriveRefreshToken'
    ...
    if (action === 'remove') {
      if (!args[1] || !args[2] || args[3]?.toLowerCase() !== 'confirm') throw new Error(`用法：${commandName} cloud remove CLOUD_NAME USER confirm`)
    ...
    if (action === 'config') {
      if (!args[1] || !args[2]) throw new Error(`用法：${commandName} cloud config get|set CLOUD_NAME USER [KEY VALUE]`)
      ...
      if (mode !== 'set' || !args[3] || !args[4] || !args[5]) throw new Error(`用法：${commandName} cloud config set CLOUD_NAME USER KEY VALUE`)
      const key = args[4]
    ...
    throw new Error(`用法：${commandName} cloud list|can-add|add|remove|config`)
  }
  ...
  private async handleDav(msg: MessageContext, args: string[]): Promise<void> {
    ...
    if (action === 'account') {
      ...
      if (accountAction === 'on' && (!config.accountUsername || !config.accountPassword)) throw new Error(`请先配置 CloudDrive 账户：${commandName} config account USER PASSWORD`)
  ...
  private async handleMutation(msg: MessageContext, args: string[]): Promise<void> {
    ...
    throw new Error(`未知文件操作，使用 ${commandName} help 查看帮助`)
  }
    ...
    cd2: async (msg: MessageContext) => {
      try {
        ...
        if (!subcommand || subcommand === 'help' || subcommand === '?') {
          ...
          await this.handleList(msg, args[1])
        } else if (subcommand === 'find' || subcommand === 'get') {
          if (!args[1]) throw new Error(`用法：${commandName} find /路径/文件`)
          await this.handleFind(msg, args[1])
        } else if (subcommand === 'search') {
          await this.handleSearch(msg, args.slice(1))
        } else if (subcommand === 'space') {
          await this.handleSpace(msg, args[1])
