...
const helpSections: HelpSection[] = [
  {
    ...
    lines: [
      ...
      `${commandName} login — 使用已保存账户登录并获取会话 Token`,
      `${commandName} status — 查看当前配置、登录和连接状态`,
      `${commandName} check — 验证 CloudDrive2 服务是否可连接`,
      `${commandName} tasks — 查看下载、上传、复制和合并任务统计`
    ]
  },
  {
    ...
    lines: [
      ...
      `${commandName} account status — 查看账户信息和服务状态`,
      `${commandName} account logout — 退出账户并清除本地登录 Token`,
      `${commandName} account reset-email 邮箱 — 发送密码重置邮件`,
      `${commandName} account reset 验证码 新密码 — 使用验证码重置密码`,
      `${commandName} account delete-email — 发送账户注销验证码邮件`,
      `${commandName} account delete 删除验证码 密码 confirm — 确认注销账户`,
      `${commandName} 2fa status — 查看两步验证是否启用`,
      `${commandName} 2fa setup 密码 — 创建两步验证密钥和二维码信息`,
      `${commandName} 2fa enable 验证码 — 启用两步验证`,
      `${commandName} 2fa disable 验证码 — 用验证码关闭两步验证`,
      `${commandName} 2fa recovery — 查看账户恢复码`,
      `${commandName} 2fa regenerate — 重新生成账户恢复码`,
      `${commandName} 2fa login 用户名 密码 验证码 — 使用 TOTP 验证码登录`,
      `${commandName} 2fa send-disable-email 邮箱 — 发送关闭两步验证邮件`,
      `${commandName} 2fa disable-email 邮件验证码 密码 — 用邮件验证码关闭两步验证`,
      `${commandName} 2fa unbind 密码 验证码 — 解绑当前设备`,
      `${commandName} session list — 列出当前账户的登录会话`,
      `${commandName} session revoke 会话编号 confirm — 撤销指定登录会话`,
      `${commandName} session revoke-others confirm — 撤销除当前会话外的其他会话`,
      `${commandName} token show — 查看当前使用的 Token，输出自动脱敏`,
      `${commandName} token clear — 清除本地保存的 Token`,
      `${commandName} token login 用户名 密码 — 使用账户登录并保存 Token`,
      `${commandName} token list — 列出 API Token，输出自动脱敏`,
      `${commandName} token info API令牌 — 查看指定 Token 的权限和状态`,
      `${commandName} token create 名称 根目录 权限 — 创建 API Token`,
      `${commandName} token modify API令牌 参数 — 修改 Token 名称、目录、权限或有效期`,
      `${commandName} token remove API令牌 confirm — 删除 API Token`
    ]
  },
  {
    title: '文件、下载、上传与挂载',
    lines: [
      ...
      `${commandName} mount update 挂载点 源目录 — 修改挂载点`,
      `${commandName} mount mount 挂载点 — 挂载指定挂载点`,
      `${commandName} mount unmount 挂载点 — 卸载指定挂载点`,
      `${commandName} mount remove 挂载点 confirm — 删除挂载点`
    ]
  },
  {
    title: '传输任务',
    lines: [
      ...
      `${commandName} transfer downloads 页码 — 查看下载任务列表`,
      `${commandName} transfer uploads 页码 — 查看上传任务列表`,
      `${commandName} transfer copies — 查看复制和移动任务`,
      `${commandName} transfer merges — 查看文件合并任务`,
      `${commandName} transfer pause all — 暂停全部下载和上传任务`,
      `${commandName} transfer resume all — 恢复全部下载和上传任务`,
      `${commandName} transfer cancel all — 取消全部下载和上传任务`,
  ...
  },
  {
    ...
    lines: [
      ...
      `${commandName} api can-add — 检查是否还能添加 Cloud API`,
      `${commandName} api remove 云盘名称 用户名 confirm — 删除 Cloud API 配置`,
      `${commandName} api config get 云盘名称 用户名 — 查看 Cloud API 配置`,
      `${commandName} api config set 云盘名称 用户名 配置项 配置值 — 修改 Cloud API 配置`,
      `${commandName} api add webdav 地址 用户名 密码 — 添加 WebDAV 云盘`,
      `${commandName} api add local 本地路径 — 添加本地目录`,
      `${commandName} api add pikpak 用户名 密码 — 添加 PikPak`,
      `${commandName} api add 115-cookie Cookie — 使用 Cookie 添加 115`,
      `${commandName} api add 115-qrcode — 扫码添加 115`,
      `${commandName} api add 189-qrcode — 扫码添加 189 云盘`,
      `${commandName} api add 115-open — 使用 115 Open OAuth 添加云盘`,
      `${commandName} api add 115-open-qrcode — 扫码添加 115 Open`,
      `${commandName} api add aliyun-oauth — 使用 OAuth 添加阿里云盘`,
      `${commandName} api add aliyun-refresh — 使用刷新令牌添加阿里云盘`,
      `${commandName} api add aliyun-qrcode — 扫码添加阿里云盘`,
      `${commandName} api add baidu-oauth — 使用 OAuth 添加百度网盘`,
      `${commandName} api add onedrive-oauth — 使用 OAuth 添加 OneDrive`,
      `${commandName} api add google-oauth — 使用 OAuth 添加 Google Drive`,
      `${commandName} api add google-refresh — 使用刷新令牌添加 Google Drive`,
      `${commandName} api add xunlei-oauth — 使用 OAuth 添加迅雷云盘`,
      `${commandName} api add xunlei-open — 使用迅雷 Open OAuth 添加云盘`,
      `${commandName} api add 123-oauth — 使用 OAuth 添加 123 云盘`,
      `${commandName} api add guangya-oauth — 使用 OAuth 添加光鸭云盘`,
      `${commandName} api add guangya-qrcode — 扫码添加光鸭云盘`,
      `${commandName} api add s3 — 使用 S3 参数添加对象存储`,
      `${commandName} api add sftp — 使用 SFTP 参数添加远程存储`,
      `${commandName} api add ftp — 使用 FTP 或 FTPS 参数添加远程存储`,
      `${commandName} api add smb — 使用 SMB 参数添加网络共享`,
      `${commandName} api add clouddrive — 添加 CloudDrive 级联存储`,
      `${commandName} api discover-smb — 发现局域网 SMB 服务器`,
      `${commandName} api discover-smb-shares 服务器 用户名 密码 — 发现 SMB 共享目录`,
      `${commandName} api oauth-state 类型回调地址 — 创建 OAuth 登录状态`
    ]
  },
  {
    title: '备份',
    lines: [
      ...
      `${commandName} backup can-add — 检查是否还能添加备份任务`,
      `${commandName} backup add 源目录 目标目录 — 添加备份任务`,
      `${commandName} backup update 源目录 目标目录 — 更新备份任务`,
      `${commandName} backup remove 源目录 confirm — 删除备份任务`,
      `${commandName} backup enable 源目录 开关 — 启用或关闭备份任务`,
      `${commandName} backup watch 源目录 开关 — 启用或关闭文件系统监听`,
      `${commandName} backup destination add 源目录 目标目录 — 添加备份目标`,
      `${commandName} backup destination remove 源目录 目标目录 — 删除备份目标`,
      `${commandName} backup status 源目录 — 查看备份状态、规则和计划`,
      `${commandName} backup strategy 源目录 替换策略 删除策略 完成策略 扫描间隔 — 设置备份策略`,
      `${commandName} backup schedule add 源目录 时间 星期 — 添加时间计划`,
      `${commandName} backup schedule clear 源目录 — 清空时间计划`,
      `${commandName} backup rule add 源目录 规则类型 规则值 黑白名单 应用范围 — 添加文件规则`,
      `${commandName} backup rule clear 源目录 — 清空文件规则`,
      `${commandName} backup restart 源目录 — 立即重新扫描备份`
    ]
  },
  {
    title: '远程上传与离线任务',
  ...
  },
  {
    ...
    lines: [
      ...
      `${commandName} system set 设置项 设置值 — 修改单项系统设置`,
      `${commandName} system set-log — 整体设置日志轮换参数`,
      `${commandName} system set-backup-limits — 整体设置备份资源限制`,
      `${commandName} system cache stats — 查看文件磁盘缓存统计`,
      `${commandName} system cache list — 列出缓存目录`,
      `${commandName} system cache purge confirm — 清理文件磁盘缓存`,
      `${commandName} system cache eviction 策略 — 设置缓存淘汰策略`,
      `${commandName} system cache folder set 路径 — 设置目录缓存规则`,
      `${commandName} system cache folder remove 路径 — 删除目录缓存规则`,
      `${commandName} system dir-cache set 路径 秒数 — 设置目录缓存时间`,
      `${commandName} system dir-cache effective 路径 — 查看有效目录缓存时间`,
      `${commandName} system dir-cache expire 路径 — 强制目录缓存过期`,
      ...
      `${commandName} system table refs 路径 — 查看引用路径`,
      ...
      `${commandName} system web set HTTP端口 HTTPS端口 开关 — 修改 Web 服务配置`,
      ...
      `${commandName} dav account on 路径 — 开启 WebDAV 账户模式`,
      `${commandName} dav account off — 关闭 WebDAV 账户模式`,
      `${commandName} dav ls 路径 — 浏览 WebDAV 目录`,
      `${commandName} dav mkdir 路径 — 创建 WebDAV 目录`,
      `${commandName} dav rm 路径 confirm — 删除 WebDAV 路径`,
      `${commandName} dav add 用户名 密码 路径 — 添加 WebDAV 用户`,
      `${commandName} dav remove 用户名 confirm — 删除 WebDAV 用户`
    ]
  }
]
